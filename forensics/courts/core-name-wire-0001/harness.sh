#!/bin/sh
# harness.sh — court CORE-NAME-WIRE-0001
#
...
case "$side" in
    oracle)
        probe="$repo/forensics/oracle/build/probe_name_wire"
        [ -x "$probe" ] || { echo "oracle probe missing; run scripts/oracle/build-oracle-probes.sh" >&2; exit 1; }
        ;;
    rust)
        probe="$repo/target/debug/probe-name-wire"
...
"$probe" < "$court_dir/inputs/wire.txt"
