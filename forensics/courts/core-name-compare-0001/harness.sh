#!/bin/sh
# harness.sh — court CORE-NAME-COMPARE-0001
...
case "$side" in
    oracle)
        probe="$repo/forensics/oracle/build/probe_name_compare"
        [ -x "$probe" ] || { echo "oracle probe missing; run scripts/oracle/build-oracle-probes.sh" >&2; exit 1; }
        ;;
    rust)
        probe="$repo/target/debug/probe-name-compare"
...
"$probe" < "$court_dir/inputs/pairs.txt"
