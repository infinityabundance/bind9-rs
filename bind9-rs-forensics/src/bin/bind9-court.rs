...
fn main() -> ExitCode {
    ...
    let result = match cmd.as_str() {
        ...
        "env-digest" => {
            println!("{}", bind9_rs_forensics::receipt::environment_digest());
            Ok(())
        }
        other => {
...
fn cmd_list() -> Result<(), String> {
    let courts = bind9_rs_forensics::court::discover(&courts_root())?;
    if courts.is_empty() {
...
fn cmd_run(args: &[String]) -> Result<(), String> {
    ...
    let oracle_status = bind9_rs_forensics::court::run_side(court, "oracle")?;
    let rust_status = bind9_rs_forensics::court::run_side(court, "rust")?;
    let mode = bind9_rs_forensics::court::compare_mode(court);
    let residuals = bind9_rs_forensics::court::compare(court, mode)?;
    let repro = format!("bind9-court run {id}");
    let receipt = bind9_rs_forensics::court::finish(court, residuals.clone(), &repro)?;
    println!(
...
fn run_side_by_id(id: &str, side: &str) -> Result<(), String> {
    ...
    let court = courts
        ...
        .find(|c| &c.id == id)
        .ok_or_else(|| format!("court {id} not found"))?;
    let status = bind9_rs_forensics::court::run_side(court, side)?;
...
fn cmd_verify(args: &[String]) -> Result<(), String> {
    ...
    let problems = bind9_rs_forensics::receipt::verify_receipt(std::path::Path::new(path))?;
...
fn cmd_index(args: &[String]) -> Result<(), String> {
    ...
    let index = bind9_rs_forensics::release_index::fetch_remote_index()?;
    bind9_rs_forensics::release_index::save_index(&index, std::path::Path::new(out))?;
    println!(
