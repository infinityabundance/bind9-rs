...
fn run_cmd(cmd: &str, args: &[String]) -> Result<(), String> {
    let dir = atlas_dir();
    let invs = bind9_rs_forensics::atlas::load_all(&dir.to_string_lossy());
    let rules =
        bind9_rs_forensics::atlas::load_rules(&dir.join("coverage-rules.json").to_string_lossy());
    let entries = bind9_rs_forensics::atlas::resolve(&invs, &rules);
    ...
    match cmd {
        ...
        "summary" => {
            for (status, count) in bind9_rs_forensics::atlas::summarize(&entries) {
        ...
        "query" => {
            ...
            let hits: Vec<_> = entries
                .iter()
                .filter(|e| bind9_rs_forensics::atlas::glob_match(glob, &e.function))
                .collect();
            if hits.is_empty() {
...
fn render_markdown(
    entries: &[bind9_rs_forensics::atlas::CoverageEntry],
    invs: &[bind9_rs_forensics::atlas::AtlasInventory],
) -> String {
