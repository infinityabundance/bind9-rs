#!/bin/sh
# compare.sh — LMDB-0001: byte-exact stdout + stderr + exit comparison.
set -eu
court="$(dirname "$0")"
cid=$(basename "$court")
    ...
    for i in range(n):
        ...
        if a != b:
            residuals.append({
                ...
                "kind": "line-mismatch",
                "description": "line {i + 1} of {surface} differs",
...
# Dependency-court pattern (fstrm/zlib/...): the comparator writes the §13
# residuals and exits 0 so the bind9-court runner's compare_custom() reads
