...
| UNKNOWN-0002 | Does BIND's masterfile lexer accept `\DDD` escapes inside quoted strings the same way as unquoted (decimal, 3 digits)? | lex.c (raw tokens), rdata.c commatxt (decimal) | 9.20.26 | ISC-LEX-0001 + WIRE-RDATA-0001 courts | **RESOLVED**: the lexer leaves `\DDD` raw inside quoted strings (only `\"` is resolved, by overwriting the backslash); the TXT character-string parser then resolves `\097` -> `a` and `\000` -> NUL inside quotes exactly as unquoted — decimal, 3 digits, ≤ 255 (byte-exact cases `TXT "with \097 escapes"` and `TXT \000`) | resolved | — |
